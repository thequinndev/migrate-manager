...
```
migrate-manager new --prefixStrategy=timestamp|date|numeric
```

### migrate-manager.yml

This command will create a new file `migrate-manager.yml` that looks like this:

```yaml
prefixStrategy: timestamp
outputDir: migrations
migrationGroupsDir: migrate-manager
splitBy:
  none:
...
1. The key `migrationGroups` inside `migrate-manager.yml` will get a new item in its array.
...
For a working example see [here](../../examples/migration-manager/migrate-manager/20250215_migrate.yml)
